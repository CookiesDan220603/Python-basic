...
def req2(f, a, b, c):  ## KHONG XOA
	try:
		...
		if dfx == zoo or dfx== nan:
			return None
		if dfy == zoo or dfy== nan:
			return None
		if dfz == zoo or dfz== nan:
			return None
		kq=dfx*(x-a)+dfy*(y-b)+dfz*(z-c)+float(fall)
		if kq == zoo or kq == nan:
			return None
	except:
		return None
...
print(req5(x**2 + 225*x*y + y**2 - 2*x -5*y + 24))
